...
class _RendementState extends State<Rendement> {
  ...
  Future<void> getComposant() async {
    final url = Uri.parse("http://192.168.1.13:8000/get_all_composant/");
  ...
  void filterData(String query) {
    setState(() {
      ...
      filteredArticles = articles?.where((article) => article["reference"].toLowerCase().contains(searchQuery)).toList();
      filteredComposants = composants?.where((composant) => composant["reference"].toLowerCase().contains(searchQuery)).toList();
    });
  }
        ...
        children: [
          ...
          Expanded(
            child: SingleChildScrollView(
              child: Column(
                children: [
                  buildDataTable("Articles", filteredArticles, columnWidth, true),
                  const SizedBox(height: 20),
                  buildDataTable("Composants", filteredComposants, columnWidth, false),
                ],
              ),
            ),
          ),
  ...
  Widget buildDataTable(String title, List<dynamic>? data, double columnWidth, bool isArticle) {
    ...
    return SingleChildScrollView(
      ...
      child: DataTable(
        ...
        border: TableBorder.all(color: const Color(0xFFC0C0C0), borderRadius: BorderRadius.circular(10)),
        columnSpacing: 20,
        columns: const [
